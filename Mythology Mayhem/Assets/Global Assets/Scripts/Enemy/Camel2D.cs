...
public class Camel2D : MonoBehaviour
{
    ...
    [SerializeField] string spitTrigger = "Spit";
    [SerializeField] float spitDistance = 6f;

    [Header("Enhanced Idle")]
    [SerializeField] string idleSitBool = "Sit";
    [SerializeField] float timeBeforeSit = 5f;
    float timeSinceLastSit = 0f;
    ...
    public void Idle()
    {
        ...
        // Continue Idle
        if (timeSinceLastSit >= timeBeforeSit && enemy.idleTimer > 0)
        {
            enemy.animator.SetBool(idleSitBool, true);
            enemy.idleTimer -= Time.deltaTime;
        }
        else if (timeSinceLastSit >= timeBeforeSit && enemy.idleTimer <= 0)
        {
            enemy.animator.SetBool(idleSitBool, false);
            enemy.idleTimer -= Time.deltaTime;
            timeSinceLastSit = 0;
        }
        else
        ...
        else
        {
            ...
            enemy.idleTimer -= Time.deltaTime;
            timeSinceLastSit += Time.deltaTime;
        }
    }//end move to target
    ...
    public void Spit()
    {
        ...
        if (enemy.DetectPlayer())
        {
            ...
            //Act
            if (Vector3.Distance(enemy.gameObject.transform.position, enemy.target) < spitDistance)
            {
                //Spit
                enemy.animator.SetBool(runBool, false);
                enemy.animator.SetTrigger(spitTrigger);
